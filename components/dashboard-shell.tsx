...
const DashboardHeaderContext = createContext<
  ((details: HeaderDetails | null) => void) | null
>(null);
          ...
          <main className="flex min-h-0 min-w-0 flex-1 flex-col bg-background md:m-2 md:rounded-xl md:shadow-sm">
            <header className="sticky top-0 z-20 flex h-12 shrink-0 items-center justify-between gap-3 border-b bg-background/95 px-4 backdrop-blur md:rounded-t-xl lg:px-6">
              ...
              {headerDetails?.action ? (
                ...
                >
                  <a
                    href={headerDetails.action.href}
                    target="_blank"
                    rel="noreferrer"
                  >
      ...
      {details?.tags?.length ? (
        <div className="hidden min-w-0 items-center gap-1.5 overflow-hidden md:flex">
          {details.tags.map((tag) => (
            <Badge
              className={cn(
                "shrink-0 bg-background text-muted-foreground",
                tag.className,
              )}
              key={tag.label}
            >
...
function SidebarContent({
  ...
  return (
    <div className="flex h-full flex-col">
      <Link
        className="mb-5 flex items-center gap-3 rounded-lg px-2 py-2"
        href="/dashboard"
      >
      ...
      <div className="mt-8">
        <div className="px-2 text-xs font-medium text-sidebar-foreground/50">
          平台
        </div>
      ...
      <div className="mt-auto grid gap-2">
        <div className="flex items-center gap-2 rounded-lg border border-sidebar-border bg-background/70 p-2">
